 ...
 * intent_detect LLM call. SAFE BY CONSTRUCTION — callers only ever route a true
 * result to the question path, NEVER to meal_log, so it can't produce a phantom
 * meal card (mirrors the 'question'-default safety rule below). The reverse
 * (keyword → meal_log) would be unsafe and is intentionally not done.
 */
